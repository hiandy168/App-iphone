/*TMODJS:{"version":2,"md5":"5b96a4af9baab46427f1b586045f4184"}*/
...
$out+=' <div class="empty-cart" style="display:none"> <div class="empty_cart_box"> <img src="../images/cart_null.png" alt="" /> <p>购物车空空如也~</p> <a class="goIndex"><button type="button">去逛逛</button></a> </div> </div> </div> <script> /* 购物车为空显示 */ if( jQuery(\'.mui-content\').find(\'.mui-row\').length == 0){ jQuery(\'.empty-cart\').show(); } /* 购物车商品删除 */ jQuery(\'.mui-content\').on(\'tap\',\'.cart-delete\',function(e){ var _that = jQuery(this); var rec_id = _that.attr(\'data-id\'); mui.confirm(\'你确定要删除吗？\',\'提示\',[\'取消\',\'确认\'], function(e){ if(e.index == 1){ jQuery.ajaxJsonp(web_url+"/mobile/flow.php",{step:\'drop_to_collect\',id:rec_id},function(data){ var uls = _that.parents(\'ul\'); if(uls.find(\'li\').length > 1){ _that.parents(\'li\').remove(); initTotal(0,0); }else{ uls.parents(\'.mui-row\').remove(); if(jQuery(\'.mui-content\').find(\'.gys\').length == 0){ jQuery(\'.empty-cart\').show(); } initTotal(0,0); } }); } }); }); /* 修改购物车数量 */ jQuery(\'.mui-content\').on(\'change\',\'input[type=number]\',function(){ var goods_number = jQuery(this).val(); var rec_id = jQuery(this).parents(\'div\').prev().attr(\'data-id\'); initTotal(rec_id, goods_number); }); /* 统计价格 */ function initTotal(r,n){ jQuery.ajaxJsonp(web_url+"/mobile/flow.php",{step:\'update_cart\',rec_id:r,goods_number:n}, function(data){ jQuery(\'.total-price\').html(data.data.total.goods_price); }); } /* 去结算 */ jQuery(\'.act-done\')[0].addEventListener(\'tap\',function(){ mui.openWindow({ url:\'./checkout.html\', id:\'checkout.html\' }); }); /*去逛逛回到首页*/ mui(".goIndex")[0].addEventListener(\'tap\',function(){ plus.webview.show("/index/jy_index.html") }) </script>';
