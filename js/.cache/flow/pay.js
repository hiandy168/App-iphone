/*TMODJS:{"version":144,"md5":"b1023224cb6023f3c699e5544f864bdd"}*/
...
$out+='</p> </li> <li class="mui-table-view-cell mui-input-row"> <label><h4>请输入密码</h4></label> <input type="password" placeholder="请输入密码"/> </li> </ul> </div> <script> jQuery(\'nav\').on(\'tap\',\'.footer_jiaru\',function(){ var password = jQuery(\'input[type=password]\').val(); var order_id = \'';
$out+=$escape(data.order.order_id);
$out+='\'; if(jQuery.trim(password) == \'\'){ mui.alert(\'密码不能为空\'); return false; } jQuery.ajaxJsonp( web_url+"/mobile/flow.php", { step:\'act_pay\', order_id:order_id, password:password }, function(data){ if(data.state == \'false\'){ jQuery.errorJudge(data, data.message); }else{ mui.alert(\'支付成功\',function(){ plus.webview.getWebviewById(".order_shiwu.html").reload(true); mui.openWindow({ url:\'../order/order_shiwu.html\' }); }); } },true); }); </script> ';
